...
export function useAppointments() {
  const { data, error, isLoading } = useSWR<Appointment[]>("appointments", fetcher, {
...
export function useMedicalHistory(petId: string) {
  const { data, error, isLoading } = useSWR<MedicalRecord[]>(`history-${petId}`, fetcher, {
    refreshInterval: 1000,
  })
...
export function addAppointmentNote(id: string, note: string) {
  appointments = appointments.map((a) => (a.id === id ? { ...a, notes: [...a.notes, note] } : a))
  mutate("appointments")
}

export function addVoiceNote(id: string, voiceNote: string) {
  appointments = appointments.map((a) => (a.id === id ? { ...a, voiceNotes: [...a.voiceNotes, voiceNote] } : a))
  mutate("appointments")
}

export function updatePetData(petId: string, updates: Partial<Pet>) {
  pets = pets.map((p) => (p.id === petId ? { ...p, ...updates } : p))
  appointments = appointments.map((a) => (a.petId === petId ? { ...a, pet: { ...a.pet, ...updates } } : a))
  mutate("pets")
  mutate("appointments")
}

export function addAppointment(appointment: Omit<Appointment, "id" | "notes" | "voiceNotes">) {
...
export function updateConsultationDetails(id: string, details: { diagnosis?: string; treatment?: string }) {
  appointments = appointments.map((a) => (a.id === id ? { ...a, ...details } : a))
  mutate("appointments")
}
...
export function finalizeConsultation(appointmentId: string) {
  ...
  appointments = appointments.map((a) => (a.id === appointmentId ? { ...a, status: "completed" as const } : a))

  // Update pet medical notes if there's a diagnosis
  if (appointment.diagnosis) {
    const summaryNote = `${new Date().toLocaleDateString()}: ${appointment.diagnosis}`
    pets = pets.map((p) =>
      p.id === appointment.petId ? { ...p, medicalNotes: [summaryNote, ...p.medicalNotes].slice(0, 5) } : p,
    )
    appointments = appointments.map((a) =>
      a.petId === appointment.petId
        ? { ...a, pet: { ...a.pet, medicalNotes: [summaryNote, ...a.pet.medicalNotes].slice(0, 5) } }
        : a,
    )
  }
